...
elseif($action === 'SAVE')
{
    ...
    if (Bitrix\Main\Loader::includeModule('kitconsulting.extendedproductsrow')) {
        $originalProductRows = !$isNew ? \Kitconsulting\ExtendedProductsRow\CCrmProductRowExtended::LoadRows('D', $ID) : array();
    } else {
    ...
    )
    {
        ...
        if (Bitrix\Main\Loader::includeModule('kitconsulting.extendedproductsrow')) {
            if(!$isNew
                && !$isCopyMode
                && $conversionWizard === null
                && \Kitconsulting\ExtendedProductsRow\CCrmProductRowExtended::AreEquals($productRows, $originalProductRows)
            )
            {
                $enableProductRows = false;
            }
    ...
    if($enableProductRows)
    {
        ...
        if(!empty($productRows))
        {
            ...
            if (
                Bitrix\Main\Loader::includeModule('kitconsulting.productapplicationsextension')
                && Bitrix\Main\Loader::includeModule('kitconsulting.extendedproductsrow')
            ) {
                $totals = \Kitconsulting\ExtendedProductsRow\CCrmProductRowExtended::CalculateTotalInfo('D', 0, false, $calculationParams, $productRows);
            } else {
