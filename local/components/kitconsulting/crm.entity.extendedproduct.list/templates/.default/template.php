...
$userFields = $GLOBALS['USER_FIELD_MANAGER']->GetUserFields(\Kitconsulting\ExtendedProductsRow\CCrmProductRowExtended::$sUFEntityID);
...
$editorConfig = [
		...
		'smartProcessSignedParameters' => $arParams['SMART_PROCESS_SIGNED_PARAMS'],
		'smartProcessId' => $arParams['SMART_PROCESS_ENTITY_TYPE_ID'],
		'smartProcessStages' => $arParams['SMART_PROCESS_STAGES'],
		'smartProcessCategories' => $arParams['SMART_PROCESS_CATEGORIES'],
		'reloadUrl' => '/local/components/kitconsulting/crm.entity.extendedproduct.list/list.ajax.php',
		...
		'minimalPriceId' => $arParams['MINIMAL_PRICE_ID']
];
...
//	echo "</pre>";
	...
	$item = [
		...
		// 'IMAGES' => $rawProduct['IMAGES'],
			...
			'UF_PURCHASE_PRICE' => $rawProduct['UF_PURCHASE_PRICE'],
			...
			'UF_RESERVE_QUANTITY' => $rawProduct['UF_RESERVE_QUANTITY'],
			...
			'MINIMAL_PRICE' => (float)($rawProduct['MINIMAL_PRICE'] ?? 0)
	];

	foreach ($userFields as $field => $fieldParams) {
		$item[$field] = $rawProduct[$field];
	}
	...
	$product['UF_PURCHASE_PRICE'] = [
			'PRICE' => [
					'NAME' => $rowId . '_PURCHASE_PRICE',
					'VALUE' => $rawProduct['UF_PURCHASE_PRICE'],
					'DISABLED' => false,
			],
			'CURRENCY' => [
					'NAME' => $rowId . '_PURCHASE_PRICE_CURRENCY',
					'VALUE' => $currency['ID'],
					'DISABLED' => true,
			],
	];
	...
	$userFieldColumns = [];
	foreach ($userFields as $field => $fieldParams) {
		if ($fieldParams['USER_TYPE_ID'] == 'boolean') {
			$isFieldDisabled = $fieldParams['EDIT_IN_LIST'] == 'N';

			if (!empty($rawProduct['UF_APPLICATION_PARENT_PRODUCT_ROW_ID'])) {
				$isFieldDisabled = true;
			}

			$product[$field] = '<div class="crm-entity-product-control-checkbox">'
					. '<input type="checkbox"'
					. ' id="' . $rowId . '_' . $field . '"'
					. ' data-field-code="' . $field . '"'
					. ' data-product-field="Y" data-parent-id="' . $rowId . '"'
					. ($rawProduct[$field] ? ' checked' : '')
					. ($isFieldDisabled ? ' disabled' : ' ')
					. '>'
					. '</div>';
		} elseif ($fieldParams['USER_TYPE_ID'] === 'money') {
			$fieldAttributes = [
					'data-field-code' => $field
			];

			if (isset($rawProduct[$field])) {
				[$productMoney, $currencyId] = explode('|', $rawProduct[$field]);
			} else {
				[$productMoney, $currencyId] = explode('|', $fieldParams['SETTINGS']['DEFAULT_VALUE']);
			...
			foreach ($grid['COLUMNS'] as &$columnArray) {
				if ($columnArray['id'] === $field) {
					$columnArray['editable'] = [
							'TYPE' => 'MONEY',
							'CURRENCY_LIST' => [
									'RUB' => 'руб.'
							],
					...
					$columnArray['align'] = 'right';
					$columnArray['default'] = true;
					break;
				}
			}
		} elseif ($field === 'UF_SMART_PROCESS_ID' &&
				!empty($rawProduct['UF_SMART_PROCESS_ID']) &&
				null !== $rawProduct['UF_SMART_PROCESS_ID'] && 'undefined' !== $rawProduct['UF_SMART_PROCESS_ID']) {
			$stage = $arParams['SMART_PROCESS_HELPER']->getSmartProcessStage($rawProduct['UF_SMART_PROCESS_ID']);
			$product['UF_SMART_PROCESS_STATUS'] = "<div style=\"color: {$stage['color']}\">{$stage['name']}</div>";
		}
	}
	// end region userfields
	...
	$freeStore =
			$rawProduct['FREE_STORE'] !== null
					? $rawProduct['FREE_STORE'] . " " . $measureName
					: '';

	$reserveInEntity =
			$rawProduct['UF_RESERVE_QUANTITY'] !== null
					? $rawProduct['UF_RESERVE_QUANTITY'] . " " . $measureName
					: '';

	$columns = [
			'MAIN_INFO' => $mainInfoColumn,
			'STORE_INFO' => HtmlFilter::encode($product['STORE_TITLE']),
			'UF_RESERVE_QUANTITY' => "<span data-name='UF_RESERVE_QUANTITY'>{$reserveInEntity}</span>",
			'DEDUCTED_INFO' => "<span data-name='DEDUCTED_QUANTITY'>{$deductedInfo}</span>",
			'ROW_RESERVED' => "<span data-name='COMMON_RESERVED'>{$rowReserved}</span>",
			'STORE_AVAILABLE' => "<a href='#' data-name='STORE_AVAILABLE'>{$storeAvailable}</a>",
			'PRICE' => $priceColumn,
			'PRICE' => $product['UF_PURCHASE_PRICE'],
			'QUANTITY' => $quantityColumn,
			'SUM' => $sumColumn,
			'DISCOUNT_PRICE' => $discountColumn,
			'DISCOUNT_ROW' => $discountRowColumn,
			'STORE_VIRTUAL' => "<span data-name='STORE_VIRTUAL'>" . ($rawProduct['STORE_VIRTUAL'] ?? 0) . "</span>",
			'STORE_GENERAL' => "<span data-name='STORE_GENERAL'>" . ($rawProduct['STORE_GENERAL'] ?? 0) . "</span>",
			'FREE_STORE' => "<span data-name='FREE_STORE'>{$freeStore}</span>",
	];
	if ($arResult['ALLOW_TAX']) {
	...
	$rowId = $rawProduct['ID'] === $productIdMask ? 'template_0' : $rawProduct['ID'];
	$rows[$rowId] = [
			...
			'columns' => $columns,
			'has_child' => false,
			'parent_id' => /*\Bitrix\Main\Grid\Context::isInternalRequest() && */
					!empty($rawProduct['UF_APPLICATION_PARENT_PRODUCT_ROW_ID']) ? $rawProduct['UF_APPLICATION_PARENT_PRODUCT_ROW_ID'] : 0,
...
?>
<div class="crm-entity-product-list--custom crm-entity-product-list-wrapper" id="<?= $containerId ?>"><?php
	if (!$isReadOnly) {
		...
		<div class="<?= $buttonTopPanelClasses ?>">
			<div>
				...
				<?php
				if ($arParams['INTERNAL_FILTER']['OWNER_TYPE'] == CCrmOwnerTypeAbbr::Deal): ?>
					<a class="ui-btn ui-btn-primary"
					   data-role="add-process-button"
					   title="<?= Loc::getMessage('CRM_ENTITY_PL_ADD_PROCESS_TITLE') ?>"
					   tabindex="-1">
						<?= Loc::getMessage('CRM_ENTITY_PL_ADD_PROCESS') ?>
					</a>
					<button type="button" class="ui-btn ui-btn-danger-light"
					   id="withdraw-reserve-button"
					   data-role="withdraw-reserve-button"
					   title="<?= Loc::getMessage('CRM_ENTITY_PL_WITHDRAW_RESERVE_TIP') ?>"
					   tabindex="-1">
						<?= Loc::getMessage('CRM_ENTITY_PL_WITHDRAW_RESERVE') ?>
					</button>
				<?php
				endif; ?>
			</div>
		...
		<div class="crm-product-list-result-container" id="<?= $productTotalContainerId ?>">
			<table class="crm-product-list-payment-side-table">
				<tr class="crm-product-list-payment-side-table-row">
					<td><?= Loc::getMessage('CRM_DEFAULT_PRODUCT_TOTAL') ?>:</td>
					<td>
						<span data-total="totalDefaultProducts">
							<?= \CCurrencyLang::CurrencyFormat($arResult['DEFAULT_PRODUCT_TOTAL'], $currency['ID'], false) ?>
						</span>
						<span data-role="currency-wrapper"
							  class="crm-product-list-result-grid-item-currency-symbol"><?= $currency['TEXT'] ?></span>
					</td>
				</tr>
				<tr class="crm-product-list-payment-side-table-row">
					<td><?= Loc::getMessage('CRM_APPLICATIONS_AND_CIRCULATION_TOTAL') ?>:</td>
					<td>
						<span data-total="totalApplicationsAndCirculation">
							<?= \CCurrencyLang::CurrencyFormat($arResult['APPLICATIONS_AND_CIRCULATION_TOTAL'], $currency['ID'], false) ?>
						</span>
						<span data-role="currency-wrapper"
							  class="crm-product-list-result-grid-item-currency-symbol"><?= $currency['TEXT'] ?></span>
					</td>
...
<script>
	...
	BX.ready(function () {
		if (!BX.Reflection.getClass('BX.Crm.Entity.ExtendedProductList.Instance')) {
			BX.Crm.Entity.ExtendedProductList.Instance = new BX.Crm.Entity.ExtendedProductList.Editor('<?=$arResult['ID']?>');
		}

		BX.Crm.Entity.ExtendedProductList.Instance.init(<?=Json::encode($editorConfig)?>);
		BX.Crm["<?=$jsEventsManagerId?>"] = BX.Crm.Entity.ExtendedProductList.Instance.getPageEventsManager();
	});
</script>
