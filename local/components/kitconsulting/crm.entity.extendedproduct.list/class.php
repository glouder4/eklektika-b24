...
if (!Loader::includeModule('kitconsulting.productapplicationsextension'))
{
    return;
}
...
final class CCrmEntityExtendedProductListComponent
	...
	implements Main\Engine\Contract\Controllerable, Main\Errorable
{
	...
	public function __construct($component = null)
	{
        ...
        $this->smartProcessHelper = new \Kitconsulting\ProductApplicationsExtension\Helper();
	}
	...
	public function onPrepareComponentParams($params): array
	{
        ...
        $advPriceResult = Catalog\GroupTable::getList([
            'select' => ['ID'],
            'filter' => ['NAME' => 'Рекламная цена']
        ])->fetch();

        if (is_array($advPriceResult)) $params['MINIMAL_PRICE_ID'] = $advPriceResult['ID'];

        if (
            $params['ENTITY_TYPE_NAME'] === CCrmOwnerType::DealName
            && !empty($params['ENTITY_ID'])
            && !empty($params['MINIMAL_PRICE_ID'])
        ) {
            $result = Crm\DealTable::getList([
                'select' => ['UF_ADV_AGENT'],
                'filter' => ['ID' => $params['ENTITY_ID']]
            ])->fetch();

            if (is_array($result) && !empty($result['UF_ADV_AGENT']))
                $params['BASE_PRICE_ID'] = $params['MINIMAL_PRICE_ID'];
        }
        $params['SMART_PROCESS_SIGNED_PARAMS'] = \Bitrix\Main\Component\ParameterSigner::signParameters('bitrix:crm.item.details', [
            'ENTITY_ID' => 0,
            'ENTITY_TYPE_ID' => $this->smartProcessHelper->getType()->getEntityTypeId()
        ]);
        $params['SMART_PROCESS_ENTITY_TYPE_ID'] = $this->smartProcessHelper->getType()->getEntityTypeId();
        $params['SMART_PROCESS_STAGES'] = $this->smartProcessHelper->getStages();
        $params['SMART_PROCESS_CATEGORIES'] = $this->smartProcessHelper->getJsonSerializableCategories();
        $params['SMART_PROCESS_HELPER'] = $this->smartProcessHelper;
		// white column list
		// black column list
	...
	public function calculateTotalDataAction(array $products, $currencyId, array $options): ?array
	{
		...
		foreach ($products as &$productRow)
		{
			...
			$productRow['UF_PURCHASE_PRICE'] = (float)($productRow['UF_PURCHASE_PRICE'] ?? 0);
            ...
            if (!empty($productRow['UF_APPLICATION_PARENT_PRODUCT_ROW_ID']) && $productRow['UF_APPLICATION_PARENT_PRODUCT_ROW_ID'] > 0) {
                $totalApplications += isset($productRow['PRICE']) ? round((float)$productRow['PRICE'] * $productRow['QUANTITY'], $this->crmSettings['PRICE_PRECISION']) : 0;
                $totalCirculation += round((float)$productRow['UF_APPLICATION_PRICE'], $this->crmSettings['PRICE_PRECISION']);
                if ($productRow['TAX_RATE'] > 0) {
					$additionalTax += (float)$productRow['UF_APPLICATION_PRICE'] -
						\Kitconsulting\ExtendedProductsRow\CCrmProductRowExtended::CalculateExclusivePrice((float)$productRow['UF_APPLICATION_PRICE'],
							$productRow['TAX_RATE']);
				}

            } else {
                $totalDefaultProducts += isset($productRow['PRICE']) ? round((float)$productRow['PRICE'] * $productRow['QUANTITY'],
					$this->crmSettings['PRICE_PRECISION']) : 0;
            }
		}
		unset($productRow);
        ...
        $totalSum += $totalCirculation;
		$totalTax = isset($result['TAX_VALUE']) ? round((float)$result['TAX_VALUE'] + $additionalTax, $this->crmSettings['PRICE_PRECISION']): 0;
		...
		$response = [
            ...
            'totalDefaultProducts' => $totalDefaultProducts,
            'totalApplicationsAndCirculation' => $totalApplications + $totalCirculation,
        ];
    ...
    public function getPriceInfoAction($productId, $basePriceId)
    {
        return [
            'basePrice' => Catalog\PriceTable::getList(
                [
                    'select' => ['*'],
                    'filter' => [
            ...
            'minimalPrice' => Catalog\PriceTable::getList(
                [
                    'select' => ['*'],
                    'filter' => [
                        '=PRODUCT_ID' => $productId,
                        '=CATALOG_GROUP_ID' => $this->arParams['MINIMAL_PRICE_ID'],
                    ]
                ]
            )->fetch()
        ];
	...
	public function calculateProductPricesAction(array $products, string $currencyId, array $options): ?array
	{
		...
		foreach ($products as $product)
		{
			...
			$response[$product['id']] = [
                ...
                'UF_PURCHASE_PRICE' => CCrmCurrency::ConvertMoney($fields['UF_PURCHASE_PRICE'], $oldCurrencyId, $currencyId),
			];
		}
	...
	protected function prepareEntitySettings(array &$params): void
	{
		...
		if (!$this->isExistErrors())
		{
			...
			if ($this->entity['ID'] > 0)
			{
				$settings = \Kitconsulting\ExtendedProductsRow\CCrmProductRowExtended::LoadSettings($this->entity['TYPE_CODE'], $this->entity['ID']);
	...
	protected function loadData(): void
	{
		...
		{
			$this->rows = \Kitconsulting\ExtendedProductsRow\CCrmProductRowExtended::LoadRows($this->entity['TYPE_CODE'], $this->entity['ID']);
		}
        ...
        if (!empty($this->arParams['MINIMAL_PRICE_ID'])) {
            foreach ($this->rows as &$row) {
                if ((int)$row['PRODUCT_ID'] > 0) {
                    $priceResult = Catalog\PriceTable::getList([
                        'select' => ['*'],
                        'filter' => [
                    ...
                    if (is_array($priceResult)) $row['MINIMAL_PRICE'] = $priceResult['PRICE'];
                }
            }
        }
	}
...
//			];
        ...
        $result['ROW_RESERVED'] = [
            'id' => 'ROW_RESERVED',
            'name' => Loc::getMessage('CRM_EXT_ENTITY_PRODUCT_LIST_COLUMN_STORE_FROM_RESERVED'),
            'title' => Loc::getMessage('CRM_EXT_ENTITY_PRODUCT_LIST_COLUMN_STORE_FROM_RESERVED'),
        ...
        $result['UF_RESERVE_QUANTITY'] = [
            'id' => 'UF_RESERVE_QUANTITY',
            'name' => Loc::getMessage('CRM_ENTITY_PRODUCT_LIST_COLUMN_RESERVED_INTO_DEAL'),
            'title' => Loc::getMessage('CRM_ENTITY_PRODUCT_LIST_COLUMN_RESERVED_INTO_DEAL'),
            'hint' => Loc::getMessage(
                'CRM_ENTITY_PRODUCT_LIST_COLUMN_STORE_FROM_RESERVED_HINT',
        ...
        $result['UF_NEED_ADJUSTMENT'] = [
            'id' => 'UF_NEED_ADJUSTMENT',
            'name' => 'Нужна приладка',
            'width' => $columnDefaultWidth
        ];

        $result['UF_CRM_PR_ADJUSTMENT'] = [
            'id' => 'UF_CRM_PR_ADJUSTMENT',
            'name' => 'Приладка',
            'width' => $columnDefaultWidth
        ];

        $result['UF_APPLICATION_PRICE'] = [
            'id' => 'UF_APPLICATION_PRICE',
            'name' => 'Настройка тиража',
            'width' => $columnDefaultWidth
        ];

        $result['UF_SMART_PROCESS_STATUS'] = [
            'id' => 'UF_SMART_PROCESS_STATUS',
            'name' => 'Статус товара',
            'width' => $columnDefaultWidth,
            'editable' => false,
        ];

        $result['STORE_VIRTUAL'] = [
            'id' => 'STORE_VIRTUAL',
            'name' => Loc::getMessage("CRM_ENTITY_PRODUCT_LIST_COLUMN_VIRTUAL_INSTOCK"),
            'width' => $columnDefaultWidth,
            'editable' => false,
            'default' => true
        ];

        $result['STORE_GENERAL'] = [
            'id' => 'STORE_GENERAL',
            'name' => Loc::getMessage("CRM_ENTITY_PRODUCT_LIST_COLUMN_REAL_INSTOCK"),
            'width' => $columnDefaultWidth,
            'editable' => false,
            'default' => true
        ];
        ...
        $result['SUPPLIER_OF_GOODS'] = [
            'id' => 'SUPPLIER_OF_GOODS',
            'name' => Loc::getMessage("CRM_ENTITY_PRODUCT_LIST_COLUMN_SUPPLIER_OF_GOODS"),
            'width' => $columnDefaultWidth
        ];

        $result['ARTIKUL_POSTAVSHCHIKA'] = [
            'id' => 'ARTIKUL_POSTAVSHCHIKA',
            'name' => Loc::getMessage("CRM_ENTITY_PRODUCT_LIST_COLUMN_ARTIKUL_POSTAVSHCHIKA"),
            'width' => $columnDefaultWidth
        ];

        $result['ARTIKUL_BITRIKS'] = [
            'id' => 'ARTIKUL_BITRIKS',
            'name' => Loc::getMessage("CRM_ENTITY_PRODUCT_LIST_COLUMN_ARTIKUL_BITRIKS"),
            'width' => $columnDefaultWidth
        ];

//        $CCrmUserType = new CCrmUserType($USER_FIELD_MANAGER, \Kitconsulting\ExtendedProductsRow\CCrmProductRowExtended::$sUFEntityID);
//        $CCrmUserType->ListAddHeaders($result);

        if (strpos($this->arParams['ENTITY_TYPE_NAME'], "DYNAMIC") !== false)
        {
            $category = $this->smartProcessHelper->getSmartProcessCategory($this->entity['ID']);

            if (!empty($category) && in_array($category->collectValues()['NAME'], [
                "Закупки",
                "Стороннее производство",
            ]))
            {
                $result['UF_PURCHASE_PRICE'] = [
                    'id' => 'UF_PURCHASE_PRICE',
                    'name' => "Цена закупки",
                    'sort' => 'UF_PURCHASE_PRICE',
                    'editable' => [
                        'TYPE' => Types::MONEY,
                        'CURRENCY_LIST' => $this->getCurrencyListForMoneyField(),
                        'PLACEHOLDER' => '0',
	...
	protected function initCrmSettings(): void
	{
		...
		if (isset($this->crmSettings['ALLOW_LD_TAX']))
		{
			...
			$totalInfo = \Kitconsulting\ExtendedProductsRow\CCrmProductRowExtended::PrepareTotalInfoFromSettings($this->entity['SETTINGS']);
    ...
    protected function getRowsStores($productsList = [])
    {
        $products = [];
        $storesList = [];
        if (!empty($productsList)) $this->rows = $productsList;
        foreach ($this->rows as $index => $row)
        {
        ...
        $stores = \CCatalogStore::GetList(
            ...
            ['ACTIVE' => 'Y', 'PRODUCT_ID' => $products, '!ELEMENT_ID' => ""],
            false,
            false,
	...
	protected function getGridRows(): array
	{
		...
		if (!empty($this->rows) && $this->isCrmFormat())
		{
            ...
            foreach ($this->rows as $index => $row)
            {
                ...
                $row['STORE_GENERAL'] = $storesList[$row['PRODUCT_ID']][self::STORE_GENERAL]['PRODUCT_AMOUNT'] ?? 0;
                $row['STORE_VIRTUAL'] = $storesList[$row['PRODUCT_ID']][self::STORE_VIRTUAL]['PRODUCT_AMOUNT'] ?? 0;
                $row['UF_RESERVE_QUANTITY'] = $row['UF_RESERVE_QUANTITY'] ?? 0;

                $row['SUPPLIER_OF_GOODS'] = $row['PROPERTY_SUPPLIER_OF_GOODS_TITLE'];
                $row['ARTIKUL_POSTAVSHCHIKA'] = $row['PROPERTY_ARTIKUL_POSTAVSHCHIKA'];
                $row['ARTIKUL_BITRIKS'] = $row['PROPERTY_ARTIKUL_BITRIKS'];
				$this->rows[$index] = $row;
			}
	...
	private function calculateRows(): void
	{
		...
		if (!empty($this->rows) && $this->isCrmFormat())
		{
			foreach ($this->rows as $index => $row)
			{
				if ($this->crmSettings['ALLOW_LD_TAX'])
				{
					$row['PRICE'] = \Kitconsulting\ExtendedProductsRow\CCrmProductRowExtended::CalculateExclusivePrice($row['PRICE'], $row['TAX_RATE']);
					$row['TAX_RATE'] = 0;
					$row['TAX_INCLUDED'] = 'N';
				}
				...
				if ($row['TAX_RATE'] != 0)
				{
					if ($row['TAX_INCLUDED'] === 'Y') //налог включён в цену
					{
				...
				{
					$row['PRICE_BRUTTO'] = \Kitconsulting\ExtendedProductsRow\CCrmProductRowExtended::CalculateInclusivePrice($row['PRICE_NETTO'], $row['TAX_RATE']);
				}
                ...
                if (!empty($row['UF_APPLICATION_PARENT_PRODUCT_ROW_ID']))
                {
                    $totalApplications += isset($row['PRICE']) ? round((float)$row['PRICE'] * $row['QUANTITY'], $this->crmSettings['PRICE_PRECISION']) : 0;
                    $totalCirculation += round((float)$row['UF_APPLICATION_PRICE'], $this->crmSettings['PRICE_PRECISION']);
					if ($row['TAX_RATE'] > 0)
					{
						$additionalTax += (float)$row['UF_APPLICATION_PRICE'] - \Kitconsulting\ExtendedProductsRow\CCrmProductRowExtended::CalculateExclusivePrice(
								(float)$row['UF_APPLICATION_PRICE'], $row['TAX_RATE']);
					}
                } else {
                    $totalDefaultProducts += isset($row['PRICE']) ? round((float)$row['PRICE'] * $row['QUANTITY'], $this->crmSettings['PRICE_PRECISION']) : 0;
                }
				$this->rows[$index] = $row;
			}
			unset($index);
        ...
        $this->arResult['DEFAULT_PRODUCT_TOTAL'] = $totalDefaultProducts;
        $this->arResult['APPLICATIONS_AND_CIRCULATION_TOTAL'] = $totalApplications + $totalCirculation;
	}
			...
			foreach ($items as $id => $list)
			{
				...
				foreach ($list as $index)
				{
					...
					if ($variation)
					{
                        ...
                        $this->rows[$index]['ROW_RESERVED'] = $replace['COMMON_STORE_RESERVED'];
					...
					)
					{
                        ...
                        $this->rows[$index]['ROW_RESERVED'] = $storeInfo['QUANTITY_RESERVED'];
					}
                    ...
                    $this->rows[$index]['FREE_STORE'] = ($this->rows[$index]['STORE_GENERAL'] - $this->rows[$index]['ROW_RESERVED']) ?? 0;
					if($this->rows[$index]['FREE_STORE'] < 0) {
						$this->rows[$index]['FREE_STORE'] = 0;
					}
    ...
    public function saveProductRowsAction($entityTypeId, $entityId, array $rows, $options = [])
	{
		CCrmProductRowExtended::setPerRowInsert(true);
		$result = CCrmProductRowExtended::SaveRows(\CCrmOwnerTypeAbbr::ResolveByTypeID($entityTypeId),
			$entityId, $rows, null, true, true, true);

		if ($result) {
			$events = GetModuleEvents('crm', 'OnAfterCrmDealProductRowsSave');
			while ($event = $events->Fetch())
				ExecuteModuleEventEx($event, [$entityId, $rows]);
		}

        if (!empty($options))
        {
            if (isset($options['STAGE']) && in_array($options['STAGE'], ["addAfterCreate"]))
            {
                $arErrorsTmp = [];
                \CBPDocument::StartWorkflow(
                    self::BP_SMART_PROCESS, /* идентификтор бизнес процесса */
        ...
        return Bitrix\Main\Engine\Response\AjaxJson::createError();
    }
    ...
    public function withdrawReserve1CAction($entityTypeId, $entityId, array $rows, $options = [])
    {
        if ($entityTypeId == \CCrmOwnerType::Deal)
        {
            $arErrorsTmp = [];
            \CBPDocument::StartWorkflow(
                self::BP_DEAL, /* идентификтор бизнес процесса */
