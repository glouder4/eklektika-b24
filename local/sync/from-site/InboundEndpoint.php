 ...
 * Keep business logic in local/sync; legacy endpoints should proxy here.
 */
class InboundEndpoint
{
    ...
    public static function processRequest(array $request): array
    {
        ...
        switch ($action) {
            case 'GET_CONTACT_ID':
                return self::handleGetContactId($request);
            case 'UPDATE_COMPANY':
                return self::handleUpdateCompany($request);
            case 'CRM_METHOD':
                return self::handleCrmMethod($request);
            default:
                return [
                    'success' => 0,
