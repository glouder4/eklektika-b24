...
// Historical migration notes are now tracked in local/sync/config/uf_mapping.php.
class CompanySync extends OutboundRequest
{
    ...
    public static function onBeforeCompanyUpdate(&$arFields): bool
    {
        ...
        $wasHeadCompany = self::isTruthy($ufValues[$ufCompanyIsHead]['VALUE'] ?? null);
        $willBeHeadCompany = \array_key_exists($ufCompanyIsHead, $arFields)
            ? self::isTruthy($arFields[$ufCompanyIsHead] ?? null)
            : $wasHeadCompany;
    ...
    public static function onAfterCompanyUpdate(&$arFields): bool
    {
        ...
        $requisitesList = \Bitrix\Crm\RequisiteTable::getList([
            'filter' => [
        ...
        $company['REQUISITES'] = $requisitesList[0] ?? [];
        self::writeOutboundTrace('CompanySync::requisites_loaded', [
            'company_id' => $companyId,
            'requisite_rows' => \count($requisitesList),
        ...
        $siteElementId = \trim(
            (string)($arFields[$ufCompanySiteElementId] ?? $company[$ufCompanySiteElementId] ?? '')
        );
        ...
        $web = (string)($company[$ufCompanyWeb] ?? '');
        $city = (string)($company[$ufCompanyCity] ?? '');
        $businessScope = (string)($company[$ufCompanyBusinessScope] ?? '');
        $legalAddress = (string)($company[$ufCompanyLegalAddress] ?? '');
        ...
        self::writeOutboundTrace('CompanySync::outbound_payload', [
            ...
            'EKLEKTIKA_SITE_URL' => \defined('EKLEKTIKA_SITE_URL') ? \EKLEKTIKA_SITE_URL : '(undefined)',
